<?php
	...
	// 时间戳
	$logisticsInfo['timestamp'] = $_GET['timestamp'];
	// 物流平台用户账号
	$logisticsInfo['user'] = $_GET['user'];
	...
	if ($mysign === $_GET['sign']){
		// 签名校验通过
		// 构建表单向请求方返回物流运单号
		$html = "<form id='logisticsReturn' name='logisticsReturn' action='notify_url.php' method='POST'>";
    	$html.= "<input type='hidden' name='order_no' value='227472389928'/>";
    	$html.= "<input type='hidden' name='carrier_name' value='".$_GET['carrier_name']."'/>";
		//submit按钮控件请不要含有name属性
    	$html = $html."<input type='submit'  value='".$button_name."' style='display:none;'></form>";
		$html = $html."<script>document.forms['logisticsReturn'].submit();</script>";
		echo $html;
	}
